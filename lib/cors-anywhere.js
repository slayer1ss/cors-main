...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    if(origin==""){origin_matched=1;}
    else{['kuponuna','canlitribun','sporyayinlari'].forEach(function(item){if(origin.includes(item)){origin_matched=1;}});}
    if(origin_matched==0){
      res.writeHead(403, 'Forbidden', cors_headers);
...
*/
    ...
    if(
        location.host.includes("savefiless.xyz")||location.host.includes("nonsavefiles.xyz")||
        location.host.includes("azureedge.net")||location.host.includes("cloudfront.net")
    ){req.headers["Referer"] = "http://liveonscore.tv";}
    else if(location.host=="lite-1x287006.com"){req.headers["Referer"] = "https://lite-1x287006.com";}
    else if(location.host=="cdn.play.live-sport24.com"){req.headers["Referer"] = "http://live-sport24.com";}
    else if(location.host=="w.cantholdus.xyz"){req.headers["Referer"] = "https://www.streameast.live";}
